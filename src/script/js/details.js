! function() {
	//1.获取sid
	var picid = location.search.substring(5).split('=')[1]; //将地址栏的sid
	//2.将当前的id传给后端获取对应的数据
	$.ajax({
		url: 'http://10.31.162.63/js1810/nubiya/php/detail.php',
		data: {
			sid: picid
		},
		dataType: 'json'
		...
		$('.mian_left_thumb img').attr('sid', data.sid);
		$('.loadtitle').html(data.title); //将标题给详情页
		$('.loadpcp').html(data.price); //将价格给详情页
		...
		//将下面小图的的索引给上面
		$('.mian_left_thumb').html(str);
		$('title').text(data.title);
		$('.mian_left_thumb li').on('click', function() {
...
//			console.log(arrsid)
	...
	$('.r-bot button').on('click', function() {
		alert(1)//点击加入购物车按钮。
		...
		//		
		//获取当前的按钮对应的商品的sid
		var $sid = $(this).parents('main_left').find('#imgSrc ').attr('sid');
